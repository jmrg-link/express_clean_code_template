...
/**
 * Quita un rol concreto a un usuario. Idempotente: si no lo tiene, no llama a
 * KC ni emite evento. Bloquea self-demotion del propio `admin`.
 */
export class RemoveRoleUseCase {
   ...
   * @param role - Rol a retirar (Zod-restringido a `USER_ROLES`).
   * @param actorKeycloakId - `req.user.id` (sub del JWT del admin).
   * @throws {CustomError} 404 si no existe; 400 si intenta quitarse su propio
   *   `admin`; 502 si KC falla.
   */
  public async execute(
    ...
    if (actorKeycloakId === user.keycloak_id && role === 'admin') {
      throw CustomError.badRequest('Cannot self-demote admin');
    }

    const after: UserRole[] = before.filter((r) => r !== role);

    await this.iam.removeRoles(user.keycloak_id, [role]);
